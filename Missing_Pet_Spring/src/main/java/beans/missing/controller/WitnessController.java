...
=======
>>>>>>> f262f5d7e4bcf1b8d9ed73a14fe7452f380f4aeb
...
>>>>>>> f262f5d7e4bcf1b8d9ed73a14fe7452f380f4aeb
	...
	// common>map.jsp 에서 마커를찍으면 생기는
	@GetMapping("/witpet/{detailAddr}/{latLng}/{missing_no}")
	...
	public Object fileUp(HttpServletRequest request, HttpServletResponse response,
			...
			System.out.println(wVO.toString());
				...
				try {
					...
					if(witService.witInfor_insert(wVO)==1)
						...
						return "redirect:/main";//저장된 VO위치정보로 마커표시하기, ajax로 wit_info.jsp띄어서 등록한 목격정보 띄우기
		...
		// 2.vo ---> DB에 저장 (pathList: insertArticle에 파일업로드경로)
		WitnessVO wVO = new WitnessVO(request.getParameter("missing_pic"), date, request.getParameter("latLng"),
				multipartRequest.getParameter("comment"), (String) request.getSession().getAttribute("loginId"),
				Integer.parseInt(no));
		// 목격자 테이블에 들어갈 정보를담은 vo ===>끝에 파라미터인자 2개는 map.jsp에저장되어있는 실종동물테이블에서 정보를 받아와야함
